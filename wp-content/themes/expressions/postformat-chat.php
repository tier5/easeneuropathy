...
if ( $display_post_icon == 1 ) {
	echo '<div class="post-icon">';
		...
		if($use_pformat_font_icons == 1 ){
			echo '<i class="post-icon-format fa fa-comments" title="chat"></i>';		
		} else {	
			echo '<img class="post-icon-format" src="'.get_template_directory_uri().'/images/format-chat_24.png" title="chat" alt="chat" />';		
		}
	...
	<?php ka_express_post_meta_top( 'chat' , $display_post_timestamp , $display_post_author , $display_post_categories ); ?>
	...
	<div class="chat-entry respimg">
		<?php the_content(__('Read more','ka_express')); ?>
	</div>
	
	<div class="clearfix"></div>
	
	<?php ka_express_post_meta_bottom( 'chat' , $display_post_tags ); ?>
