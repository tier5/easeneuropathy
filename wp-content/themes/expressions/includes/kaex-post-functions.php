 ...
 */
if( !function_exists( 'ka_express_post_meta_top' ) ) {
	 function ka_express_post_meta_top( $post_format , $display_post_timestamp , $display_post_author , $display_post_categories ) {
	 		
	 	global $kaex_options;
		$use_font_icons = $kaex_options['kaex_use_font_icons'];
		
		if ( $display_post_timestamp == 1 || $display_post_author == 1 || $display_post_categories == 1 ) {
			
			if( $post_format == 'aside' ) {

				if($display_post_author == 1) {
					
					if( is_sticky() ) {
						
						if( $use_font_icons == 1 ) {
							echo '<i class="kaex-sticky fa fa-thumb-tack" title="sticky">&nbsp;</i>';
						} else {
					...
					<span class="author vcard">
						<a class="url fn n" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" 
							title="<?php _e('Posts by Author','ka_express'); ?>" 
				...
				<div class="postmetatop">
					...
					if( $display_post_timestamp == 1 ) {
						
						if( $use_font_icons != 1 ) { ?>
							<span class="timestamp updated"><?php the_time(get_option('date_format')); ?></span>
						<?php } else { ?>
							<span class="timestamp updated"><i class="fa fa-clock-o" title="<?php _e('Published Date','ka_express'); ?>">&nbsp;</i><?php the_time(get_option('date_format')); ?></span>
						<?php }
					}
					...
					if($display_post_author == 1) {
						...
						if( $use_font_icons != 1 ) { ?>
							...
							<span class="author vcard"><?php _e('By:','ka_express'); ?>&nbsp;
								<a class="url fn n" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" 
									title="<?php _e('Posts by Author','ka_express'); ?>" 
									rel="author" ><?php the_author(); ?> </a>
							</span>
						...
						<?php } else { ?>
							...
							<span class="author vcard"><i class="fa fa-user" title="<?php _e('By','ka_express'); ?>">&nbsp;</i>
								<a class="url fn n" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" 
									title="<?php _e('Posts by Author','ka_express'); ?>" 
									rel="author" ><?php the_author(); ?> </a>
							</span>
					...
					if($display_post_categories == 1) {
						
						if( $use_font_icons != 1 ) { ?>
							<span class="categories"><?php _e('In:','ka_express'); ?>&nbsp;<?php the_category(', '); ?></span>
						<?php } else { ?>
							<span class="categories"><i class="fa fa-bookmark" title="<?php _e('Categories','ka_express'); ?>">&nbsp;</i>
 ...
 * documentation please
 */
if( !function_exists( 'ka_express_post_meta_bottom' ) ) {
	function ka_express_post_meta_bottom( $post_format , $display_post_tags ) { 
		...
		if( $post_format == 'aside' || $post_format == 'status' ) { ?>
			...
			<div class="postmetabottom">
				...
				<?php } else { ?>
					<span class="edit"> <?php
						$edit_before = '<i class="fa fa-edit" title="'.__('Edit','ka_express').'">&nbsp;</i>';
						edit_post_link($edit_before); ?>
					</span>
				<?php } ?>
			</div>
		...
		<?php } else { ?>
			...
			<div class="postmetabottom">
				...
				<?php } else { ?>
					...
					<span class="pagelist">
						<?php 
							$page_text = '<i class="fa fa-files-o" title="'.__('Pages','ka_express').'"></i>';
					...
					<?php if($display_post_tags == 1) { ?>
						<span class="taglist"> 
							<?php
							$tags_before = '<i class="fa fa-tags" title="'.__('Tags','ka_express').'">&nbsp;</i>';
							the_tags($tags_before);
							?>
						</span>
					<?php } ?>
						
					<?php if(get_the_title()=="") echo '<span class="no-title"><a class="fa fa-link" href="'.get_permalink().'" rel="bookmark" title="'.__('Link to Post','ka_exprss').'" >'.__(" Link to Post","ka_express").'</a></span>'; ?>
